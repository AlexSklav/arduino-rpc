...
def get_protobuf_fields_frame(message_type) -> pd.DataFrame:
    """
    ...
    Each row of the result data frame contains the following values:
      ...
      - `parent_name`: Name of the submessage in the parent message ('' for top level tags).
      - `parent_field`: Protocol Buffer field descriptor of the submessage in the parent message
        (`None` for top level tags).
      - `field_name`: Name of field/tag.
      - `field_desc`: Protocol Buffer field descriptor.
    """
    frames = []

    def _frames(root, parent_field: str = None) -> None:
        atom_fields = [(n, f) for n, f in root.fields_by_name.items() if not f.type == f.TYPE_MESSAGE]
        if atom_fields:
            frame = pd.DataFrame(atom_fields, columns=['field_name', 'field_desc'])
            frame = frame.assign(**{'root_name': message_type.DESCRIPTOR.name,
                                    'msg_name': root.name,
                                    'msg_desc': root,
                                    'parent_name': parent_field.name if parent_field else '',
                                    'parent_field': parent_field})
            frames.append(frame)

        for n, f in root.fields_by_name.items():
            if f.type == f.TYPE_MESSAGE:
                _frames(f.message_type, parent_field=f)

    _frames(message_type.DESCRIPTOR)
    # column_order = ['root_name', 'msg_name', 'msg_desc', 'parent_name',
    #                 'parent_field', 'field_name', 'field_desc']
    return pd.concat(frames)  # .loc[:, column_order].copy()


def extract_callback_data(df_protobuf: pd.DataFrame, method_name: str) -> Tuple[pd.DataFrame, pd.Series]:
    """
    ...
    Each row of the parent data frame contains the following values:

      - `msg_name`: Protocol Buffer submessage name.
      - `msg_desc`: Protocol Buffer message descriptor instance.
      - `parent_name`: Name of the submessage in the parent message
        ('' for top level tags).
    ...
    df_parents = (df_protobuf[df_protobuf.parent_name.isin(parents)]
                  .loc[:, ['msg_name', 'msg_desc', 'parent_name', 'parent_field']].drop_duplicates()
                  .set_index('parent_name')).loc[parents]

    s_field = df_protobuf[(df_protobuf.parent_name == parents[-1]) & (df_protobuf.field_name == field)].iloc[0]
    s_field['atom_type'] = TYPE_CALLABLE_MAP[s_field.field_desc.type]
    s_field.name = field
    return df_parents, s_field


def get_field_value(root, field_descriptor, full_name: str, set_default: Optional[bool] = False) -> Union[None, str]:
    """
...
def resolve_field_values(message, set_default: Optional[bool] = False) -> pd.DataFrame:
    """
    ...
    df_fields['full_name'] = df_fields['parent_name'].map(lambda v: v + '.' if v else '') + df_fields['field_name']

    df_fields['value'] = (df_fields[['field_desc', 'full_name']]
                          .apply(lambda v: get_field_value(message, *v.values, set_default=set_default), axis=1))
